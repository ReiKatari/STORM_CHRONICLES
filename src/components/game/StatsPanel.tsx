...
import { getSetById } from '@/game/items';
    ...
    <div className="bg-slate-900/95 rounded-2xl border border-slate-700/60 p-3.5 shadow-2xl backdrop-blur-md space-y-3 font-sans">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-2">
        ...
        <span className={`text-xs font-black px-3 py-1 rounded-xl border font-mono transition-all ${
          ...
          {fmt(points)} {points === 1 ? 'очко характеристик' : points > 1 && points < 5 ? 'очка характеристик' : 'очков характеристик'}
        </span>
      </div>
          ...
          <div key={d.id} className="group relative flex items-center gap-2 bg-slate-950/80 rounded-xl px-2.5 py-1 border border-slate-800/80 hover:border-slate-700 transition-all">
            ...
            <div className="flex gap-1">
              <button
                ...
                disabled={points <= 0}
                className="w-5 h-5 rounded-md bg-amber-600 hover:bg-amber-500 disabled:opacity-30 text-white text-xs font-black leading-none flex items-center justify-center cursor-pointer"
              >+</button>
              <button
                onClick={() => allocate10(d.id)}
                disabled={points <= 0}
                className="px-1.5 h-5 rounded-md bg-amber-600 hover:bg-amber-500 disabled:opacity-30 text-white text-[9px] font-black leading-none flex items-center justify-center cursor-pointer"
              >+10</button>
            </div>
      ...
      <div className="pt-2.5 border-t border-slate-800 grid grid-cols-2 gap-x-3 gap-y-1.5 text-[11px] text-slate-300 font-medium bg-slate-950/60 p-2.5 rounded-xl border border-slate-800">
        <div>⚔️ Урон: <b className="text-white font-mono font-extrabold">{fmt(derived.dmgMin || 0)}–{fmt(derived.dmgMax || 0)}</b></div>
        <div>✨ Сила Способностей: <b className="text-white font-mono font-extrabold">{fmt(derived.skillPower || 0)}</b></div>
        <div>🛡️ Броня: <b className="text-white font-mono font-extrabold">{fmt(derived.armor || 0)}</b></div>
        <div>💥 Критический Удар: <b className="text-white font-mono font-extrabold">{(derived.critChance || 0).toFixed(1)}%</b></div>
        <div>⚡ Скорость Атаки: <b className="text-white font-mono font-extrabold">{(derived.attackSpeed || 1).toFixed(2)} в секунду</b></div>
        <div>💨 Уклонение: <b className="text-white font-mono font-extrabold">{(derived.dodge || 0).toFixed(1)}%</b></div>
        <div>💰 Бонус Золота: <b className="text-amber-300 font-mono font-extrabold">+{(derived.goldBonus || 0).toFixed(0)}%</b></div>
        <div>📈 Бонус Опыта: <b className="text-sky-300 font-mono font-extrabold">+{(derived.xpBonus || 0).toFixed(0)}%</b></div>
        <div>🍀 Шанс Добычи: <b className="text-lime-300 font-mono font-extrabold">+{(derived.dropBonus || 0).toFixed(0)}%</b></div>
        <div>🩸 Вампиризм: <b className="text-red-300 font-mono font-extrabold">{(derived.lifestealPct || derived.lifesteal || 0)}%</b></div>
      </div>
      ...
      <div className="p-3 rounded-xl bg-indigo-950/40 border border-indigo-500/40 space-y-2">
        ...
        <div className="grid grid-cols-3 gap-1.5 text-[10px] text-center font-bold">
          ...
          <div className="p-1.5 rounded-lg bg-slate-950/90 border border-slate-800 text-red-300 font-mono">
            ...
            <div className="text-[8px] text-slate-400 font-sans font-normal">Здоровье</div>
          </div>
        </div>

        {/* Active Set Bonuses Preview */}
        {activeSets.length > 0 && (
          <div className="pt-1.5 border-t border-indigo-500/30 space-y-1">
            <div className="text-[9.5px] font-black text-emerald-400 uppercase tracking-wider">
              ✨ Комплекты ({activeSets.length}):
            </div>
            {activeSets.map(({ setDef, count }) => (
              <div key={setDef.id} className="text-[9px] flex items-center justify-between text-slate-300">
                <span style={{ color: setDef.color }}>{setDef.icon} {setDef.name}</span>
                <span className="text-amber-300 font-mono font-bold">{count}/{setDef.pieces.length}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {onOpenPaperdoll && (
          <button
            ...
            className="w-full mt-1.5 py-2 px-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold text-xs shadow-lg transition-all active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer"
          >
