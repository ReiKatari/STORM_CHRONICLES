...
export default function TalentsPanel() {
  ...
  const filteredTalents = talentsDef.filter(t => {
    const branchPoints = branchPointsMap[t.branchId] ?? 0;
    const reqPoints = t.row * 2;
    const isUnlocked = t.row === 0 || branchPoints >= reqPoints;
    ...
    <div className="bg-slate-900/95 rounded-2xl border border-slate-700/60 p-4 flex flex-col h-full min-h-0 shadow-2xl backdrop-blur-md space-y-3 font-sans w-full">
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none shrink-0">
        <button
          ...
          className={`py-1.5 px-3 rounded-xl font-extrabold text-xs transition-all shrink-0 border ${
            activeBranch === 'all'
        ...
        {branches.map(b => {
          ...
          return (
            <button
              ...
              className={`py-1.5 px-3 rounded-xl font-extrabold text-xs flex items-center gap-1.5 transition-all shrink-0 border ${
                isActive
                  ? 'bg-slate-800 border-slate-600 text-white shadow'
                  : 'bg-slate-950/60 border-slate-800 text-slate-400 hover:text-slate-200'
              }`}
      ...
      {/* Talent Cards Grid (Matching SkillsPanel spacious structure) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 overflow-y-auto flex-1 min-h-0 pr-1 w-full">
            ...
            >
                ...
                <div className="flex items-start gap-2.5">
                  <div
                    ...
                    style={{
                      ...
                      boxShadow: rank > 0 ? `0 0 12px ${branch?.color || '#a855f7'}40` : undefined,
                    }}
                  >
                    {t.icon}
                  </div>
                  ...
                  <div className="min-w-0 flex-1">
                    <h4 className="font-black text-xs text-slate-100 leading-tight">{t.name}</h4>
                    <p className="text-[11px] text-slate-300 leading-snug mt-1">{t.desc}</p>
                  </div>
                </div>
              </div>

              {/* Footer Bonus Effect & Upgrade Action */}
              <div className="flex items-center justify-between gap-2 pt-2 border-t border-slate-800/80">
                <div className="min-w-0 flex-1">
                  ...
                  {!isUnlocked && (
                    <span className="text-[9.5px] text-red-400 font-semibold block">
                ...
                {can && (
                  <button
                    onClick={() => learn(t.id)}
                    className="text-[10.5px] font-black px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white shadow-md active:scale-95 shrink-0"
                  >
                    +1 Изучить
                  </button>
                )}
